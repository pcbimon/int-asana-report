...
function DialogContent({
  ...
  // size -> class map; these override width/max-width behavior from base
  const sizeClassMap: Record<string, string> = {
    ...
    threeQuarter: 'w-3/4 max-w-none',
    full: 'w-full max-w-none',
  }

  const sizeClass = size ? sizeClassMap[size] ?? '' : 'w-full max-w-[calc(100%-2rem)] sm:max-w-lg'
  ...
  return (
    <DialogPortal data-slot="dialog-portal">
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          // base classes (no width/max-width) - sizing comes from `sizeClass` so callers can override
